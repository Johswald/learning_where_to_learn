...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir', type=str,
                        ...
                        help='Where should all the output files be written ' +
                             'to? Note, a timestep is added to this path, ' +
                             'except "force_out_dir" is set. ' +
                             'Default: %(default)s.')
    parser.add_argument('--force_out_dir', action='store_true',
                        help='If enabled, the search will be stored in the ' +
                             'exact location provided in "out_dir" and not a ' +
                             'subfolder.')
    parser.add_argument('--dont_force_new_dir', action='store_true',
                        help='If enabled, the search can be stored in an '+
                             ...
                             'this option is not a merging option. Previous ' +
                             'hpsearch results will be overwritten if ' +
                             'existing.')
    ...
    while i < len(old_cmds):
        ...
        # Check if current command has dublicate.
        if i < len(old_cmds)-1:
            hash_next, cmd_next = old_cmds[i+1]
            if hash_i == hash_next:
                warnings.warn('Command dublicate found! The following ' +
                    'commands have been identified as dublicates. The first ' +
                    'one will be removed.\n--- %s\n--- %s' \
